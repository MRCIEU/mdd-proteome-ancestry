...
load(file.path("C:/Users/HP OMEN GAMING/Desktop/mr-uganda-hackathon-Final/results", "eur_druggable.rdata"))
eur <- res
eur$pop <- "EUR"

#african ancestry(afr)
load(file.path("C:/Users/HP OMEN GAMING/Desktop/mr-uganda-hackathon-Final/results", "afr_druggable.rdata"))
afr<- res
afr$pop <- "AFR"

#central south asian ancestry(csa)
load(file.path("C:/Users/HP OMEN GAMING/Desktop/mr-uganda-hackathon-Final/results", "sas_druggable.rdata"))
sas<- res
sas$pop <- "SAS"

#east asian ancestry(eas)
load(file.path("C:/Users/HP OMEN GAMING/Desktop/mr-uganda-hackathon-Final/results", "eas_druggable.rdata"))
eas<- res
eas$pop <- "EAS"
...
##writing out data
eur_report <- eur[, c(4,7,8,9,10)]
write.csv(eur_report, "results/eur_druggable_report.csv", row.names = FALSE)


afr_report <- afr[, c(4,7,8,9,10)]
write.csv(afr_report, "results/afr_druggable_report.csv", row.names = FALSE)


sas_report <- sas[, c(4,7,8,9,10)]
write.csv(sas_report, "results/sas_druggable_report.csv", row.names = FALSE)


eas_report <- eas[, c(4,5,7,8,9)]
write.csv(eas_report, "results/eas_druggable_report.csv", row.names = FALSE)
...
ggsave(file=file.path("C:/Users/HP OMEN GAMING/Desktop/mr-uganda-hackathon-Final/results", "combined_druggable.png"), width=10, height=10)
```
...
ggplot(combined_data, aes(y=pop, x=b, color=pop)) +
  ...
  geom_errorbarh(aes(xmin=b-se*1.96, xmax=b+se*1.96), height=0) +
  geom_vline(xintercept=0, linetype="dashed") +
  labs(title = "Multi ancestry forest plot of druggable MDD drug targets",
       x = "beta_IV (logodds ratio per SD)",
       y = "Proteins") +
  facet_grid(exposure ~ ., scales = "free_y", space="free_y") +
  theme_minimal() +  
...
ggsave(file=file.path("C:/Users/HP OMEN GAMING/Desktop/mr-uganda-hackathon-Final/results", "combined_druggable_Q.png"), width=10, height=10)
ggsave(file=file.path("C:/Users/HP OMEN GAMING/Desktop/mr-uganda-hackathon-Final/results", "combined_druggable_Q.pdf"), width=10, height=10)
...
ggplot(combined_data, aes(y=pop, x=b, color=pop)) +
  ...
  geom_errorbarh(aes(xmin=b-se*1.96, xmax=b+se*1.96), height=0) +
  geom_vline(xintercept=0, linetype="dashed") +
  labs(title = "Multi ancestry forest plot of druggable MDD gene targets",
       x = "Beta (logodds ratio per SD)",
       y = "Proteins") +
  facet_grid(exposure ~ ., scales = "free_y", space="free_y") +
  theme_minimal() +  
  theme(
  ...
  scale_color_manual(values=custom_colors) +
  scale_size_manual(name="Heterogeniety",
    values=c(`TRUE` = 5, `FALSE` = 2), 
    breaks = c(TRUE, FALSE),
    labels = c("Yes (pvalue< 0.05)", "No (pvalue>= 0.05)"))
...
write.csv(combined_data_sorted, "C:/Users/HP OMEN GAMING/Desktop/mr-uganda-hackathon-Final/results/combined_druggable_MR_Q_.csv", row.names = FALSE)
