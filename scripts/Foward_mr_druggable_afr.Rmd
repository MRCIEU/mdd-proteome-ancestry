...
```{r}
mdd<-fread("C:/Users/HP OMEN GAMING/Desktop/mr-uganda-hackathon-Final/scratch/FE_afr_exclu_whi_jhs_23andMe_qced_rsid.txt.gz")
head(mdd)
```
creating a code column in mdd and filtering for positions in common with eqtl
```{r}
...
```
Keep just best SNP for each druggable protein in the harmonised data
```{r}
dat <- dat %>%
  ...
  slice_head(n=1)
dim(dat)
write.csv(dat, "C:/Users/HP OMEN GAMING/Desktop/mr-uganda-hackathon-Final/results/AFR_druggable_SNP.csv", row.names = FALSE)
...
```{r}
res <- mr(dat)
save(res, file=file.path("C:/Users/HP OMEN GAMING/Desktop/mr-uganda-hackathon-Final/results", "afr_druggable.rdata"))
